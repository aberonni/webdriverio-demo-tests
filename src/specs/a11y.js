...
describe('A11y testing with axe example', function () {
    it('should verify that there are no critical a11y violations', function () {
        ...
        // make sure that the browser has enough time to execute axe - it may take a while
        ...
        // filtering out non-critical violations - it would have been nice to avoid this
        // but I couldn't find a website that didn't have any violations
        const violations = value.violations
            .filter(v => v.impact === 'critical');

        assert.equal(violations.length, 0, 'Expected no critical a11y violations');
    });
});
